...
from dearbaby.decorators import permission_required, assert_can_manage, _current_user
from dearbaby.roles import manageable_roles, PERMISSIONS, HIERARCHY
...
from dearbaby.models import (Report, Post, Comment, ModerationAction, Announcement,
                        Subscription, Payment, Child, Milestone, JournalEntry,
                        Media, Album)
from dearbaby.roles import can_manage
from dearbaby.helpers import json_body, paginate
...
def user_detail(user_id):
    """
    Everything an admin legitimately needs about an account — and nothing
    more.

    WHAT THIS RETURNS: identity, role, status, subscription and billing
    history, activity COUNTS, public community posts, and moderation history.

    WHAT IT DELIBERATELY DOES NOT RETURN: journal entries, photos,
    ultrasound scans, milestone notes, growth measurements, or any other
    memory content. Those belong to the family, not to the operator of the
    service. An admin can see THAT a user has 47 photos; never what is in
    them.

    This is not an oversight to be "fixed" later. A pregnancy and baby app
    holds medical images and photographs of children. Blanket operator
    access to that is the kind of thing that ends a company. If support
    genuinely needs to see a specific item, the right mechanism is a
    consented, time-boxed, audit-logged access grant requested FROM the
    user — not an admin god-mode switch.
    """
    target = db.session.get(User, user_id)
    if target is None:
        return jsonify(error="User not found"), 404
    ...
    actor = _current_user()
    child_ids = [c.id for c in Child.alive().filter_by(owner_id=user_id).all()]

    subs = (Subscription.query.filter_by(user_id=user_id)
            .order_by(Subscription.created_at.desc()).all())
    payments = (Payment.alive().filter_by(user_id=user_id)
                .order_by(Payment.created_at.desc()).limit(20).all())

    posts = (Post.alive().filter_by(author_id=user_id)
             .order_by(Post.created_at.desc()).limit(20).all())

    actions = (ModerationAction.query.filter_by(entity_type="user", entity_id=user_id)
               .order_by(ModerationAction.created_at.desc()).limit(20).all())
    reports_against = Report.alive().filter_by(entity_type="user",
                                               entity_id=user_id).count()

    return jsonify(
        user=target.to_dict(full=True),
        # can THIS actor act on THIS target? drives the buttons in the UI
        manageable=can_manage(actor.role, target.role) and actor.id != target.id,
        # counts only — never the content behind them
        activity={
            "children": len(child_ids),
            "milestones": _count(Milestone, child_ids),
            "journal_entries": _count(JournalEntry, child_ids),
            "media_items": _count(Media, child_ids),
            "albums": _count(Album, child_ids),
            "community_posts": len(posts),
        },
        subscriptions=[s.to_dict() for s in subs],
        payments=[p.to_dict() for p in payments],
        # public content only — this is already visible to every other user
        posts=[{"id": p.id, "title": p.title, "kind": p.kind, "status": p.status,
                "created_at": p.created_at.isoformat()} for p in posts],
        moderation={"reports_against": reports_against,
                    "actions": [{"action": a.action, "reason": a.reason,
                                 "created_at": a.created_at.isoformat()}
                                for a in actions]},
        privacy_note="Memory content (journal text, photos, scans, milestone "
                     "notes) is never exposed to administrators.",
    ), 200


def _count(model, child_ids):
    """Row count for a child-scoped model, without touching its content."""
    if not child_ids:
        return 0
    return model.alive().filter(model.child_id.in_(child_ids)).count()
